...
// WATCH
gulp.task('watch', [<% if (answers.jade) { -%>'jade',<% } -%>'inject-all'], function () {
...
<% if (answers.jade) { -%>
      '.tmp/*/templates/*.html', // each module's compiled jade into html
<% } -%>
      'app/*/assets/**/*'
    ])
...
// SERVE TASKS
gulp.task('serve', [<% if (answers.jade) { -%>'jade', <% } -%>,'inject-all'], function () {
